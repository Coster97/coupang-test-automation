...
//LOGIN_EMAIL_TC003
test("LOGIN_EMAIL_TC003: 비밀번호 필드가 비어 있을 시 오류 메시지 표시", async ({
  page,
}) => {
  const loginPage = new LoginPage(page);
...
// //LOGIN_EMAIL_TC004 -> 데이터 요청 실패
// test("LOGIN_EMAIL_TC004: 올바른 이메일 + 틀린 비밀번호 입력 시 오류 메시지 표시", async ({
//   page,
// }) => {
//   const loginPage = new LoginPage(page);
...
// //LOGIN_EMAIL_TC005 -> 데이터 요청 실패
// test("LOGIN_EMAIL_TC005: 가입하지 않은 이메일 + 비밀번호 입력 시 오류 메시지 표시", async ({
//   page,
// }) => {
//   const loginPage = new LoginPage(page);

//   await loginPage.goto();
//   await loginPage.login("dmsrud15012@naver.com", "test1357!");
//   await loginPage.getIntegrationErrorMessage();
...
//LOGIN_EMAIL_TC006
test("LOGIN_EMAIL_TC006: 이메일 필드에 입력 후 ‘x’ 버튼 클릭 시 입력값 삭제", async ({
  page,
}) => {
  const loginPage = new LoginPage(page);
...
//LOGIN_EMAIL_TC007
test("LOGIN_EMAIL_TC007: 비밀번호 필드 눈 아이콘 클릭 시 암호화 해제", async ({
  page,
}) => {
  const loginPage = new LoginPage(page);
...
// //LOGIN_EMAIL_TC008 -> 데이터 요청 실패
// test("LOGIN_EMAIL_TC008: 올바른 이메일과 비밀번호 입력 후 로그인 성공", async ({
//   page,
// }) => {
//   const loginPage = new LoginPage(page);
